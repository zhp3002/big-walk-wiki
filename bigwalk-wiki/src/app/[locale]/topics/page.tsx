...
import { localCategory } from '../../../data/i18n';
    ...
    <>
      ...
      <section className="section">
        <div className="container">
          ...
          <p style={{ color: 'var(--text-dim)' }}><strong style={{ color: 'var(--cream)' }}>Stage 1:</strong> {t('stage1')}</p>
          <p style={{ color: 'var(--text-dim)' }}><strong style={{ color: 'var(--cream)' }}>Stage 2:</strong> {t('stage2')}</p>
          <p style={{ color: 'var(--text-dim)' }}><strong style={{ color: 'var(--cream)' }}>Stage 3:</strong> {t('stage3')}</p>

          {/* JUMP */}
          <div style={{ marginTop: 28, display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {categories.map((c) => (
              <Link key={c.slug} href={href(`/topics#${c.slug}`)} className="btn" style={{ padding: '0.4em 0.9em', fontSize: '0.72rem' }}>{localCategory(c, locale).name}</Link>
            ))}
          </div>
        </div>
      </section>
      ...
      {categories.map((cat) => {
        ...
        const lc = localCategory(cat, locale);
        return (
          <section key={cat.slug} id={cat.slug} className="section">
            <div className="container">
              <div className="kicker kicker-dim">{lc.name}</div>
              <h2 className="section-h2">{t('catHeading', { name: lc.name.toUpperCase() })}</h2>
              ...
              <p className="lead" style={{ fontSize: '0.95rem' }}>{lc.lead}</p>
